...
import com.mabphun.HotelServer.services.admin.rooms.RoomsService;
...
public class RoomsController {
    ...
    public ResponseEntity<?> postRoom(@RequestBody RoomDto dto){
        boolean success = roomsService.postRoom(dto);
        if(success){
    ...
    public ResponseEntity<?> updateRoom(@PathVariable Long id, @RequestBody RoomDto dto){
        boolean success = roomsService.updateRoom(id, dto);
        if(success){
            return ResponseEntity.status(HttpStatus.OK).build();
        }
        else{
